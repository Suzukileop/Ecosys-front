...
// Experience's own default title is a long sentence ("{years}+ years of
// hands-on experience in my field.") that reliably splits into two balanced
// lines — that's the whole point of this design. The plain section title
// (usually one or two short words) would stay on a single line and never
// show the two-line serif treatment, so this design gets its own default
// sentence, same as Masthead's own default headline.
const DEFAULT_TITLE_TEXT = 'A curated collection of work, built with care.';

/** Two-line split, avoiding a one-word widow on the last line — same rule as
 *  Experience's `splitSerifLeadLines`. */
...
/**
 * Serif lead — exact disposition + motion copy of Experience's Table header
 * (`TableExperienceHeader`): a small uppercase label above a large serif
...
export function WorkSerifLeadHeader({
  ...
  const labelTone = workPaletteTokenColor(presentation.serifLeadLabelColor ?? 'texteFort');
  const titleTone = workPaletteTokenColor(presentation.serifLeadTitleColor ?? 'texteFort');
  const subtitleTone = workPaletteTokenColor(presentation.serifLeadSubtitleColor ?? 'texteFort');
  const labelFontSize = LABEL_SIZE[presentation.serifLeadLabelSize ?? 'md'];
  const labelFontWeight = LABEL_WEIGHT[presentation.serifLeadLabelWeight ?? 'regular'];
  const titleFontSize = TITLE_SIZE[presentation.serifLeadTitleSize ?? 'md'];
  const titleFontWeight = TITLE_WEIGHT[presentation.serifLeadTitleWeight ?? 'regular'];
  const subtitleFontSize = SUBTITLE_SIZE[presentation.serifLeadSubtitleSize ?? 'md'];
  const subtitleFontWeight = SUBTITLE_WEIGHT[presentation.serifLeadSubtitleWeight ?? 'regular'];
  ...
  useLayoutEffect(() => {
    ...
    const scroller = workHeaderScrollParent(header);
