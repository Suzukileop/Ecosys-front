...
const DEFAULT_WORD_1 = 'Visual';
const DEFAULT_WORD_2 = 'Gallery';
const MARQUEE_REPEATS = 4;
const MARQUEE_SPEED_PX = 44; // Experience's "medium" default
const MARQUEE_GAP = '1rem'; // "md" default

const WORD_SIZE: Record<PortfolioGalleryHeaderTitleSize, string> = {
...
function GalleryHeaderMarqueeTrack({
  words,
  ink,
  fontSize,
  hidden = false,
 ...
 */
export function GalleryHeaderMarqueeHeader({
  ...
  const ink = galleryHeaderPaletteTokenColor(presentation.headerMarqueeWordColor ?? 'principal');
  ...
  useLayoutEffect(() => {
    ...
    const scroller = galleryHeaderScrollParent(root);
    ...
    >
      ...
      >
        <div ref={rowRef} className="flex w-max whitespace-nowrap will-change-transform">
          <GalleryHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} />
          <GalleryHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} hidden />
        </div>
      </div>
