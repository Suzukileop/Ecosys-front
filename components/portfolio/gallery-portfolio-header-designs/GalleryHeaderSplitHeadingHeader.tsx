...
import { DEFAULT_GALLERY_PRESENTATION, type PortfolioGalleryPresentationSettings } from '@/components/portfolio/portfolio-gallery-settings';
import {
...
const DEFAULT_LABEL_TEXT = 'Gallery';
const DEFAULT_TITLE_TEXT = 'Visual journal';
...
const LABEL_SIZE_CLASS: Record<PortfolioGalleryHeaderTitleSize, string> = {
  sm: 'text-[0.6rem]',
  md: 'text-[0.7rem]',
  lg: 'text-[0.82rem]',
  xl: 'text-[0.95rem]',
...
const TITLE_WEIGHT: Record<PortfolioGalleryHeaderTitleWeight, { italic: number; regular: number }> = {
...
const LABEL_WEIGHT_CLASS: Record<PortfolioGalleryHeaderTitleWeight, string> = {
  light: 'font-normal',
  regular: 'font-medium',
...
/**
 * Split heading — exact disposition + motion copy of Portfolio/Work's Split
 * heading header (`WorkSplitHeadingHeader`): the title on the left (first
 * word italic/light, rest regular) masked-reveals from the bottom; a small
 ...
 */
export function GalleryHeaderSplitHeadingHeader({
  ...
  const ink = galleryHeaderPaletteTokenColor(presentation.headerSplitHeadingTitleColor ?? 'principal');
  const muted = galleryHeaderPaletteTokenColor(presentation.headerSplitHeadingLabelColor ?? 'secondaire');
  const titleWeight = TITLE_WEIGHT[presentation.headerSplitHeadingTitleWeight ?? 'regular'];
  ...
  return (
    <div
      ...
      data-gallery-header="split-heading"
    >
      <div className="flex flex-wrap items-start justify-between gap-x-8 gap-y-4">
        {title ? (
          <div className="overflow-hidden">
            <h2
              ref={titleRef}
              className={`max-w-4xl ${TITLE_SIZE_CLASS[presentation.headerSplitHeadingTitleSize ?? 'md']} leading-[1.06] tracking-tight`}
        ...
        <span
          ref={labelRef}
          className={`whitespace-nowrap uppercase ${LABEL_SIZE_CLASS[presentation.headerSplitHeadingLabelSize ?? 'md']} ${LABEL_WEIGHT_CLASS[presentation.headerSplitHeadingLabelWeight ?? 'regular']}`}
          style={{
            color: muted,
