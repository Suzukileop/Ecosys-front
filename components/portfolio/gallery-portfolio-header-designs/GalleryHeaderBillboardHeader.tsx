...
const DEFAULT_BIG_WORD = 'GALLERY';
const DEFAULT_COUNT_TEXT = '{count} images — visual journal below';
...
function GalleryHeaderBillboardBigWord({
  text,
  tone,
  wordStyle,
}: {
  text: string;
  tone: string;
  wordStyle: PortfolioGalleryHeaderBillboardWordStyle;
}) {
 ...
 */
export function GalleryHeaderBillboardHeader({
  ...
  const wordTone = galleryHeaderPaletteTokenColor(presentation.headerBillboardWordColor ?? 'principal');
  const titleTone = galleryHeaderPaletteTokenColor(presentation.headerBillboardTitleColor ?? 'principal');
  const metaTone = galleryHeaderPaletteTokenColor(presentation.headerBillboardMetaColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const bigTitle = section.querySelector('[data-gallery-header-gsap-big-title]') as HTMLElement | null;
      const secondaryTitle = section.querySelector('[data-gallery-header-gsap-secondary]') as HTMLElement | null;
      const metaEl = section.querySelector('[data-gallery-header-gsap-meta]') as HTMLElement | null;
  ...
  return (
    <div
      ...
      data-gallery-header="billboard"
    >
      <div style={getInitialBigTitleStyle()} data-gallery-header-gsap-big-title>
        <GalleryHeaderBillboardBigWord text={bigWord} tone={wordTone} wordStyle={presentation.headerBillboardWordStyle ?? 'outline'} />
      </div>
      ...
      <div className="mt-8 flex flex-col gap-5 sm:mt-10 sm:flex-row sm:items-end sm:justify-between sm:gap-10">
        <h2
          ...
          data-gallery-header-gsap-secondary
        >
          {renderSecondaryTitle}
        </h2>

        <div className="flex max-w-xs flex-col items-start gap-3 sm:items-end" style={getInitialMetaStyle()} data-gallery-header-gsap-meta>
          {countText ? (
