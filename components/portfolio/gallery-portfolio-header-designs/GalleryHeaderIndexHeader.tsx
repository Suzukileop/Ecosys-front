...
const DEFAULT_SUBTITLE_TEXT = 'Images, films, and chosen moments.';
const COUNT_DURATION_MS = 800;
const EASE = 'cubic-bezier(0.16, 1, 0.3, 1)';

/** Label is a small uppercase kicker — sizes stay compact at every step. */
const LABEL_SIZE: Record<PortfolioGalleryHeaderTitleSize, string> = {
  sm: '0.6rem',
  md: '0.68rem',
  lg: '0.76rem',
  xl: '0.85rem',
};
const LABEL_WEIGHT: Record<PortfolioGalleryHeaderTitleWeight, number> = {
  light: 400,
  regular: 600,
  semibold: 700,
  bold: 800,
};
const TITLE_SIZE: Record<PortfolioGalleryHeaderTitleSize, string> = {
  sm: 'clamp(1.9rem, 4.4vw, 3.1rem)',
  md: 'clamp(2.3rem, 5.3vw, 3.85rem)',
  lg: 'clamp(2.75rem, 6.2vw, 4.6rem)',
  xl: 'clamp(3.2rem, 7.1vw, 5.4rem)',
};
const TITLE_WEIGHT: Record<PortfolioGalleryHeaderTitleWeight, number> = {
  light: 300,
  regular: 300,
  semibold: 500,
  bold: 650,
};
const SUBTITLE_SIZE: Record<PortfolioGalleryHeaderTitleSize, string> = {
  sm: '0.8125rem',
  md: '0.9375rem',
  lg: '1.0625rem',
  xl: '1.1875rem',
};
const SUBTITLE_WEIGHT: Record<PortfolioGalleryHeaderTitleWeight, number> = {
  light: 350,
  regular: 400,
  semibold: 500,
  bold: 600,
...
/**
 * Index — a ledger-style divider rule up top, a counting numeral (the gallery
 * item count itself, ticking up from zero) on the left, split from the
 ...
 */
export function GalleryHeaderIndexHeader({
  ...
  const countLabel = countLabelCustom || (count === 1 ? 'Item' : 'Items');

  const labelTone = galleryHeaderPaletteTokenColor(presentation.headerIndexLabelColor ?? 'texteFort');
  const numberTone = galleryHeaderPaletteTokenColor(presentation.headerIndexNumberColor ?? 'principal');
  const titleTone = galleryHeaderPaletteTokenColor(presentation.headerIndexTitleColor ?? 'texteFort');
  const subtitleTone = galleryHeaderPaletteTokenColor(presentation.headerIndexSubtitleColor ?? 'texteFort');
  ...
  return (
    <header
      ref={headerRef}
      className="pf-gallery-header-index-header relative w-full text-left"
