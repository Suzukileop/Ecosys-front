...
const DEFAULT_LINE_1 = 'Visual stories.';
const DEFAULT_LINE_2 = 'Chosen with intent.';
const DEFAULT_LINE_3 = 'Captured over time.';

const HEADLINE_SIZE_CLASS: Record<PortfolioGalleryHeaderTitleSize, string> = {
...
const HEADLINE_WEIGHT: Record<PortfolioGalleryHeaderTitleWeight, { light: number; heavy: number }> = {
 ...
 * line.
 */
export function GalleryHeaderMastheadHeader({
  ...
  const ink = galleryHeaderPaletteTokenColor(presentation.headerMastheadHeadlineColor ?? 'principal');
  ...
  // word) creates fresh `[data-gallery-header-word]` spans that the effects
  ...
  useEffect(() => {
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || !animationEnabled) {
      section.querySelectorAll<HTMLElement>('[data-gallery-header-word]').forEach((word) => {
    ...
    const triggerAnimation = () => {
      ...
      const lineEls = section.querySelectorAll<HTMLElement>('[data-gallery-header-line]');
      const delay = 100;

      lineEls.forEach((line, index) => {
        const words = line.querySelectorAll<HTMLElement>('[data-gallery-header-word]');
      ...
      {styledLines.length > 0 ? (
        <div className={`flex flex-col gap-1 sm:gap-2 ${centered ? 'items-center' : 'items-start'}`}>
          {styledLines.map((words, lineIndex) => (
            <div key={lineIndex} className="overflow-hidden" data-gallery-header-line>
              <h2
