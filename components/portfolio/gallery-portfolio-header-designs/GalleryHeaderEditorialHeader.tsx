...
function galleryHeaderScrollParent(el: HTMLElement | null): HTMLElement | undefined {
...
const TITLE_SIZE_CLASS: Record<PortfolioGalleryHeaderTitleSize, string> = {
...
const TITLE_WEIGHT_CLASS: Record<PortfolioGalleryHeaderTitleWeight, string> = {
  light: '!font-thin',
  regular: '!font-light',
 ...
 */
export function GalleryHeaderEditorialHeader({
  title,
  subtitle,
  ...
  const presentation = presentationProp ?? DEFAULT_GALLERY_PRESENTATION;
  const centered = presentation.headerDesignAlignment === 'center';
  const animationEnabled = presentation.headerAnimationEnabled !== false;
  const titleText = title.trim();
  const subtitleText = subtitle?.trim() || '';
  const titleInk = galleryTitleColorStyle(presentation.titleColor).color as string;
  ...
  useLayoutEffect(() => {
    ...
    try {
      ctx = gsap.context(() => {
        ...
        const kicker = pick('.pf-gallery-header-editorial-kicker');
        const lines = pick('.pf-gallery-header-editorial-title-line');
        const subs = pick('.pf-gallery-header-editorial-sub');
        const stage = header.querySelector<HTMLElement>('.pf-gallery-header-editorial-stage');
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            trigger: header,
            scroller: galleryHeaderScrollParent(header),
            start: 'top 85%',
            once: true,
          },
        });
      ...
      // with it (every other section on the page, not just Gallery) — this is why the page
      ...
      console.error('[GalleryHeaderEditorialHeader] GSAP entrance animation failed to initialize', error);
      ctx?.revert();
      gsap.set(
        header.querySelectorAll(
          '.pf-gallery-header-editorial-kicker, .pf-gallery-header-editorial-title-line, .pf-gallery-header-editorial-sub'
        ),
        { clearProps: 'all' }
      );
    }
  ...
  return (
    <header
      ref={headerRef}
      className={`pf-gallery-header-editorial-header relative w-full ${centered ? 'text-center' : 'text-left'}`}
      style={{ marginBottom: `${GALLERY_HEADER_MARGIN_BOTTOM_REM[presentation.headerMarginBottom ?? 'md']}rem` }}
    >
      <div className={`pf-gallery-header-editorial-stage flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between ${centered ? 'sm:flex-col sm:items-center' : ''}`}>
        <div className="min-w-0 max-w-3xl">
          <p
            className="pf-gallery-header-editorial-kicker mb-0 text-[0.68rem] font-semibold uppercase tracking-[0.24em]"
            style={{ color: kickerInk }}
          >
            Gallery
          </p>
          <h2
            className={`mb-0 mt-3 ${galleryHeaderDesignFontClass(presentation.titleFont, 'title')} ${TITLE_SIZE_CLASS[presentation.headerTitleSize ?? 'md']} ${TITLE_WEIGHT_CLASS[presentation.headerTitleWeight ?? 'regular']} tracking-[-0.03em] lg:leading-[0.98]`}
            style={galleryTitleColorStyle(presentation.titleColor)}
          >
            <span className="pf-gallery-header-editorial-title-mask block overflow-hidden">
              <span className="pf-gallery-header-editorial-title-line block">{titleText}</span>
            </span>
          </h2>
          {subtitleText ? (
            <p
              className={`pf-gallery-header-editorial-sub mb-0 mt-6 max-w-xl ${galleryHeaderDesignFontClass(presentation.subtitleFont, 'subtitle')} text-sm leading-relaxed sm:text-base`}
              style={gallerySubtitleColorStyle(presentation.subtitleColor)}
            >
              {subtitleText}
            </p>
          ) : null}
