...
function InfoHeaderMarqueeTrack({
  words,
  ink,
  fontSize,
  hidden = false,
 ...
 */
export function InfoHeaderMarqueeHeader({
  ...
  const ink = infoHeaderPaletteTokenColor(presentation.headerMarqueeWordColor ?? 'principal');
  ...
  useLayoutEffect(() => {
    ...
    const scroller = infoHeaderScrollParent(root);
    ...
    >
      ...
      >
        <div ref={rowRef} className="flex w-max whitespace-nowrap will-change-transform">
          <InfoHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} />
          <InfoHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} hidden />
        </div>
      </div>
