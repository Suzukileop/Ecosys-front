...
function infoHeaderScrollParent(el: HTMLElement | null): HTMLElement | undefined {
...
const TITLE_SIZE_CLASS: Record<PortfolioInfoHeaderTitleSize, string> = {
...
const TITLE_WEIGHT_CLASS: Record<PortfolioInfoHeaderTitleWeight, string> = {
  light: '!font-thin',
  regular: '!font-light',
 ...
 */
export function InfoHeaderEditorialHeader({
  title,
  subtitle,
  ...
  const titleInk = infoTitleColorStyle(presentation.titleColor).color as string;
  ...
  useLayoutEffect(() => {
    ...
    try {
      ctx = gsap.context(() => {
        ...
        const kicker = pick('.pf-info-header-editorial-kicker');
        const lines = pick('.pf-info-header-editorial-title-line');
        const subs = pick('.pf-info-header-editorial-sub');
        const stage = header.querySelector<HTMLElement>('.pf-info-header-editorial-stage');
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            trigger: header,
            scroller: infoHeaderScrollParent(header),
            start: 'top 85%',
            once: true,
          },
        });
      ...
      console.error('[InfoHeaderEditorialHeader] GSAP entrance animation failed to initialize', error);
      ctx?.revert();
      gsap.set(
        header.querySelectorAll(
          '.pf-info-header-editorial-kicker, .pf-info-header-editorial-title-line, .pf-info-header-editorial-sub'
        ),
        { clearProps: 'all' }
      );
    }
  ...
  return (
    <header
      ref={headerRef}
      className={`pf-info-header-editorial-header relative w-full ${centered ? 'text-center' : 'text-left'}`}
      style={{ marginBottom: `${INFO_HEADER_MARGIN_BOTTOM_REM[presentation.headerMarginBottom ?? 'md']}rem` }}
    >
      <div className={`pf-info-header-editorial-stage flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between ${centered ? 'sm:flex-col sm:items-center' : ''}`}>
        <div className="min-w-0 max-w-3xl">
          <p
            className="pf-info-header-editorial-kicker mb-0 text-[0.68rem] font-semibold uppercase tracking-[0.24em]"
            style={{ color: kickerInk }}
          >
            Info
          </p>
          <h2
            className={`mb-0 mt-3 ${infoHeaderDesignFontClass(presentation.titleFont, 'title')} ${TITLE_SIZE_CLASS[presentation.headerTitleSize ?? 'md']} ${TITLE_WEIGHT_CLASS[presentation.headerTitleWeight ?? 'regular']} tracking-[-0.03em] lg:leading-[0.98]`}
            style={infoTitleColorStyle(presentation.titleColor)}
          >
            <span className="pf-info-header-editorial-title-mask block overflow-hidden">
              <span className="pf-info-header-editorial-title-line block">{titleText}</span>
            </span>
          </h2>
          {subtitleText ? (
            <p
              className={`pf-info-header-editorial-sub mb-0 mt-6 max-w-xl ${infoHeaderDesignFontClass(presentation.subtitleFont, 'subtitle')} text-sm leading-relaxed sm:text-base`}
              style={infoSubtitleColorStyle(presentation.subtitleColor)}
            >
              {subtitleText}
            </p>
          ) : null}
