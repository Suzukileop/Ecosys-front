...
function InfoHeaderBillboardBigWord({
  text,
  tone,
  wordStyle,
}: {
  text: string;
  tone: string;
  wordStyle: PortfolioInfoHeaderBillboardWordStyle;
}) {
 ...
 */
export function InfoHeaderBillboardHeader({
  ...
  const wordTone = infoHeaderPaletteTokenColor(presentation.headerBillboardWordColor ?? 'principal');
  const titleTone = infoHeaderPaletteTokenColor(presentation.headerBillboardTitleColor ?? 'principal');
  const metaTone = infoHeaderPaletteTokenColor(presentation.headerBillboardMetaColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const bigTitle = section.querySelector('[data-info-header-gsap-big-title]') as HTMLElement | null;
      const secondaryTitle = section.querySelector('[data-info-header-gsap-secondary]') as HTMLElement | null;
      const metaEl = section.querySelector('[data-info-header-gsap-meta]') as HTMLElement | null;
  ...
  return (
    <div
      ...
      data-info-header="billboard"
    >
      <div style={getInitialBigTitleStyle()} data-info-header-gsap-big-title>
        <InfoHeaderBillboardBigWord text={bigWord} tone={wordTone} wordStyle={presentation.headerBillboardWordStyle ?? 'outline'} />
      </div>
      ...
      <div className="mt-8 flex flex-col gap-5 sm:mt-10 sm:flex-row sm:items-end sm:justify-between sm:gap-10">
        <h2
          ...
          data-info-header-gsap-secondary
        >
          {renderSecondaryTitle}
        </h2>

        <div className="flex max-w-xs flex-col items-start gap-3 sm:items-end" style={getInitialMetaStyle()} data-info-header-gsap-meta>
          {countText ? (
