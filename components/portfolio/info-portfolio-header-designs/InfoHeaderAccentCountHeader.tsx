...
const SIZE_CLASS: Record<PortfolioInfoHeaderTitleSize, string> = {
...
const LEAD_WEIGHT_CLASS: Record<PortfolioInfoHeaderTitleWeight, string> = {
  light: 'font-normal',
  regular: 'font-medium',
...
/**
 * Accent count — a badge (visible info methods count) inline with a lead
 ...
 */
export function InfoHeaderAccentCountHeader({
  ...
  const badgeColor = infoHeaderPaletteTokenColor(presentation.headerAccentCountBadgeColor ?? 'principal');
  const leadColor = infoHeaderPaletteTokenColor(presentation.headerAccentCountLeadColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const badge = section.querySelector<HTMLElement>('[data-info-header-badge]');
      const leadEl = section.querySelector<HTMLElement>('[data-info-header-lead]');
