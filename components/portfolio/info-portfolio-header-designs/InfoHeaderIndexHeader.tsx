...
const DEFAULT_SUBTITLE_TEXT = 'Background, education, and how I work.';
const COUNT_DURATION_MS = 800;
const EASE = 'cubic-bezier(0.16, 1, 0.3, 1)';

/** Label is a small uppercase kicker — sizes stay compact at every step. */
const LABEL_SIZE: Record<PortfolioInfoHeaderTitleSize, string> = {
  sm: '0.6rem',
  md: '0.68rem',
  lg: '0.76rem',
  xl: '0.85rem',
};
const LABEL_WEIGHT: Record<PortfolioInfoHeaderTitleWeight, number> = {
  light: 400,
  regular: 600,
  semibold: 700,
  bold: 800,
...
const SUBTITLE_SIZE: Record<PortfolioInfoHeaderTitleSize, string> = {
  sm: '0.8125rem',
  md: '0.9375rem',
  lg: '1.0625rem',
  xl: '1.1875rem',
};
const SUBTITLE_WEIGHT: Record<PortfolioInfoHeaderTitleWeight, number> = {
  light: 350,
  regular: 400,
  semibold: 500,
  bold: 600,
 ...
 * visible info methods count itself, ticking up from zero) on the left,
 * split from the title by a vertical rule. Strictly two phases, driven by
 * React state (not left to CSS/GSAP timing to get out of sync with
 * re-renders): first the counter runs on its own, and only once it fully
 * finishes does the title/subtitle on the right fade in — never before,
 * never partially.
 */
export function InfoHeaderIndexHeader({
  ...
  const countLabel = countLabelCustom || (count === 1 ? 'Channel' : 'Channels');

  const labelTone = infoHeaderPaletteTokenColor(presentation.headerIndexLabelColor ?? 'texteFort');
  const numberTone = infoHeaderPaletteTokenColor(presentation.headerIndexNumberColor ?? 'principal');
  const titleTone = infoHeaderPaletteTokenColor(presentation.headerIndexTitleColor ?? 'texteFort');
  const subtitleTone = infoHeaderPaletteTokenColor(presentation.headerIndexSubtitleColor ?? 'texteFort');
  ...
  return (
    <header
      ref={headerRef}
      className="pf-info-header-index-header relative w-full text-left"
