...
const DEFAULT_LINE_1 = 'Get to know me.';
const DEFAULT_LINE_2 = 'A bit of my story.';
const DEFAULT_LINE_3 = 'Background, skills, and how I work.';

const HEADLINE_SIZE_CLASS: Record<PortfolioInfoHeaderTitleSize, string> = {
...
const HEADLINE_WEIGHT: Record<PortfolioInfoHeaderTitleWeight, { light: number; heavy: number }> = {
 ...
 * line.
 */
export function InfoHeaderMastheadHeader({
  ...
  const ink = infoHeaderPaletteTokenColor(presentation.headerMastheadHeadlineColor ?? 'principal');
  ...
  // word) creates fresh `[data-info-header-word]` spans that the effects
  ...
  useEffect(() => {
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || !animationEnabled) {
      section.querySelectorAll<HTMLElement>('[data-info-header-word]').forEach((word) => {
    ...
    const triggerAnimation = () => {
      ...
      const lineEls = section.querySelectorAll<HTMLElement>('[data-info-header-line]');
      const delay = 100;

      lineEls.forEach((line, index) => {
        const words = line.querySelectorAll<HTMLElement>('[data-info-header-word]');
      ...
      {styledLines.length > 0 ? (
        <div className={`flex flex-col gap-1 sm:gap-2 ${centered ? 'items-center' : 'items-start'}`}>
          {styledLines.map((words, lineIndex) => (
            <div key={lineIndex} className="overflow-hidden" data-info-header-line>
              <h2
