...
const DEFAULT_LINE_1 = 'Core stack.';
const DEFAULT_LINE_2 = 'Chosen with intent.';
const DEFAULT_LINE_3 = 'Kept up to date.';

const HEADLINE_SIZE_CLASS: Record<PortfolioStackHeaderTitleSize, string> = {
...
const HEADLINE_WEIGHT: Record<PortfolioStackHeaderTitleWeight, { light: number; heavy: number }> = {
 ...
 * line.
 */
export function StackHeaderMastheadHeader({
  ...
  // word) creates fresh `[data-stack-header-word]` spans that the effects
  ...
  useEffect(() => {
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || !animationEnabled) {
      section.querySelectorAll<HTMLElement>('[data-stack-header-word]').forEach((word) => {
    ...
    const triggerAnimation = () => {
      ...
      const lineEls = section.querySelectorAll<HTMLElement>('[data-stack-header-line]');
      const delay = 100;

      lineEls.forEach((line, index) => {
        const words = line.querySelectorAll<HTMLElement>('[data-stack-header-word]');
      ...
      {styledLines.length > 0 ? (
        <div className={`flex flex-col gap-1 sm:gap-2 ${centered ? 'items-center' : 'items-start'}`}>
          {styledLines.map((words, lineIndex) => (
            <div key={lineIndex} className="overflow-hidden" data-stack-header-line>
              <h2
