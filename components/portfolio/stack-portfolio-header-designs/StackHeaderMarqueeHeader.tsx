...
const DEFAULT_WORD_1 = 'Core';
const DEFAULT_WORD_2 = 'Stack';
const MARQUEE_REPEATS = 4;
const MARQUEE_SPEED_PX = 44; // Experience's "medium" default
const MARQUEE_GAP = '1rem'; // "md" default

const WORD_SIZE: Record<PortfolioStackHeaderTitleSize, string> = {
...
function StackHeaderMarqueeTrack({
  words,
  ink,
  fontSize,
  hidden = false,
 ...
 */
export function StackHeaderMarqueeHeader({
  ...
  const ink = stackHeaderPaletteTokenColor(presentation.headerMarqueeWordColor ?? 'principal');
  ...
  useLayoutEffect(() => {
    ...
    const scroller = stackHeaderScrollParent(root);
    ...
    >
      ...
      >
        <div ref={rowRef} className="flex w-max whitespace-nowrap will-change-transform">
          <StackHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} />
          <StackHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} hidden />
        </div>
      </div>
