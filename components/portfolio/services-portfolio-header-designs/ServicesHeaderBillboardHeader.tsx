...
function ServicesHeaderBillboardBigWord({
  text,
  tone,
  wordStyle,
}: {
  text: string;
  tone: string;
  wordStyle: PortfolioServicesHeaderBillboardWordStyle;
}) {
 ...
 */
export function ServicesHeaderBillboardHeader({
  ...
  const wordTone = servicesHeaderPaletteTokenColor(presentation.headerBillboardWordColor ?? 'principal');
  const titleTone = servicesHeaderPaletteTokenColor(presentation.headerBillboardTitleColor ?? 'principal');
  const metaTone = servicesHeaderPaletteTokenColor(presentation.headerBillboardMetaColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const bigTitle = section.querySelector('[data-services-header-gsap-big-title]') as HTMLElement | null;
      const secondaryTitle = section.querySelector('[data-services-header-gsap-secondary]') as HTMLElement | null;
      const metaEl = section.querySelector('[data-services-header-gsap-meta]') as HTMLElement | null;
  ...
  return (
    <div
      ...
      data-services-header="billboard"
    >
      <div style={getInitialBigTitleStyle()} data-services-header-gsap-big-title>
        <ServicesHeaderBillboardBigWord text={bigWord} tone={wordTone} wordStyle={presentation.headerBillboardWordStyle ?? 'outline'} />
      </div>
      ...
      <div className="mt-8 flex flex-col gap-5 sm:mt-10 sm:flex-row sm:items-end sm:justify-between sm:gap-10">
        <h2
          ...
          data-services-header-gsap-secondary
        >
          {renderSecondaryTitle}
        </h2>

        <div className="flex max-w-xs flex-col items-start gap-3 sm:items-end" style={getInitialMetaStyle()} data-services-header-gsap-meta>
          {countText ? (
