...
const WORD_SIZE: Record<PortfolioServicesHeaderTitleSize, string> = {
...
function ServicesHeaderMarqueeTrack({
  words,
  ink,
  fontSize,
  hidden = false,
 ...
 */
export function ServicesHeaderMarqueeHeader({
  ...
  useLayoutEffect(() => {
    ...
    const scroller = servicesHeaderScrollParent(root);
    ...
    >
      ...
      >
        <div ref={rowRef} className="flex w-max whitespace-nowrap will-change-transform">
          <ServicesHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} />
          <ServicesHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} hidden />
        </div>
      </div>
