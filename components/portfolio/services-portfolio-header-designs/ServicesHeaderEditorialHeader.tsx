...
function servicesHeaderScrollParent(el: HTMLElement | null): HTMLElement | undefined {
...
const TITLE_SIZE_CLASS: Record<PortfolioServicesHeaderTitleSize, string> = {
...
const TITLE_WEIGHT_CLASS: Record<PortfolioServicesHeaderTitleWeight, string> = {
  light: '!font-thin',
  regular: '!font-light',
 ...
 */
export function ServicesHeaderEditorialHeader({
  title,
  subtitle,
  ...
  const titleInk = servicesTitleColorStyle(presentation.titleColor).color as string;
  ...
  useLayoutEffect(() => {
    ...
    try {
      ctx = gsap.context(() => {
        ...
        const kicker = pick('.pf-services-header-editorial-kicker');
        const lines = pick('.pf-services-header-editorial-title-line');
        const subs = pick('.pf-services-header-editorial-sub');
        const stage = header.querySelector<HTMLElement>('.pf-services-header-editorial-stage');
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            trigger: header,
            scroller: servicesHeaderScrollParent(header),
            start: 'top 85%',
            once: true,
          },
        });
      ...
      console.error('[ServicesHeaderEditorialHeader] GSAP entrance animation failed to initialize', error);
      ctx?.revert();
      gsap.set(
        header.querySelectorAll(
          '.pf-services-header-editorial-kicker, .pf-services-header-editorial-title-line, .pf-services-header-editorial-sub'
        ),
        { clearProps: 'all' }
      );
    }
  ...
  return (
    <header
      ref={headerRef}
      className={`pf-services-header-editorial-header relative w-full ${centered ? 'text-center' : 'text-left'}`}
      style={{ marginBottom: `${SERVICES_HEADER_MARGIN_BOTTOM_REM[presentation.headerMarginBottom ?? 'md']}rem` }}
    >
      <div className={`pf-services-header-editorial-stage flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between ${centered ? 'sm:flex-col sm:items-center' : ''}`}>
        <div className="min-w-0 max-w-3xl">
          <p
            className="pf-services-header-editorial-kicker mb-0 text-[0.68rem] font-semibold uppercase tracking-[0.24em]"
            style={{ color: kickerInk }}
          >
            Services
          </p>
          <h2
            className={`mb-0 mt-3 ${servicesHeaderDesignFontClass(presentation.titleFont, 'title')} ${TITLE_SIZE_CLASS[presentation.headerTitleSize ?? 'md']} ${TITLE_WEIGHT_CLASS[presentation.headerTitleWeight ?? 'regular']} tracking-[-0.03em] lg:leading-[0.98]`}
            style={servicesTitleColorStyle(presentation.titleColor)}
          >
            <span className="pf-services-header-editorial-title-mask block overflow-hidden">
              <span className="pf-services-header-editorial-title-line block">{titleText}</span>
            </span>
          </h2>
          {subtitleText ? (
            <p
              className={`pf-services-header-editorial-sub mb-0 mt-6 max-w-xl ${servicesHeaderDesignFontClass(presentation.subtitleFont, 'subtitle')} text-sm leading-relaxed sm:text-base`}
              style={servicesSubtitleColorStyle(presentation.subtitleColor)}
            >
              {subtitleText}
            </p>
          ) : null}
