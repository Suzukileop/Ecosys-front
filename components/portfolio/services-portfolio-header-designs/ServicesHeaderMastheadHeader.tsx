...
const DEFAULT_LINE_1 = 'Real solutions.';
const DEFAULT_LINE_2 = 'Built with care.';
const DEFAULT_LINE_3 = 'Delivered on time.';

const HEADLINE_SIZE_CLASS: Record<PortfolioServicesHeaderTitleSize, string> = {
...
const HEADLINE_WEIGHT: Record<PortfolioServicesHeaderTitleWeight, { light: number; heavy: number }> = {
 ...
 * line.
 */
export function ServicesHeaderMastheadHeader({
  ...
  const ink = servicesHeaderPaletteTokenColor(presentation.headerMastheadHeadlineColor ?? 'principal');
  ...
  // word) creates fresh `[data-services-header-word]` spans that the effects
  ...
  useEffect(() => {
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || !animationEnabled) {
      section.querySelectorAll<HTMLElement>('[data-services-header-word]').forEach((word) => {
    ...
    const triggerAnimation = () => {
      ...
      const lineEls = section.querySelectorAll<HTMLElement>('[data-services-header-line]');
      const delay = 100;

      lineEls.forEach((line, index) => {
        const words = line.querySelectorAll<HTMLElement>('[data-services-header-word]');
      ...
      {styledLines.length > 0 ? (
        <div className={`flex flex-col gap-1 sm:gap-2 ${centered ? 'items-center' : 'items-start'}`}>
          {styledLines.map((words, lineIndex) => (
            <div key={lineIndex} className="overflow-hidden" data-services-header-line>
              <h2
