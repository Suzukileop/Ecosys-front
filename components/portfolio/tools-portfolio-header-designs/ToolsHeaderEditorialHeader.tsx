...
function toolsHeaderScrollParent(el: HTMLElement | null): HTMLElement | undefined {
...
const TITLE_SIZE_CLASS: Record<PortfolioToolsHeaderTitleSize, string> = {
...
const TITLE_WEIGHT_CLASS: Record<PortfolioToolsHeaderTitleWeight, string> = {
  light: '!font-thin',
  regular: '!font-light',
 ...
 */
export function ToolsHeaderEditorialHeader({
  title,
  subtitle,
  ...
  const titleInk = toolsTitleColorStyle(presentation.titleColor).color as string;
  ...
  useLayoutEffect(() => {
    ...
    try {
      ctx = gsap.context(() => {
        ...
        const kicker = pick('.pf-tools-header-editorial-kicker');
        const lines = pick('.pf-tools-header-editorial-title-line');
        const subs = pick('.pf-tools-header-editorial-sub');
        const stage = header.querySelector<HTMLElement>('.pf-tools-header-editorial-stage');
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            trigger: header,
            scroller: toolsHeaderScrollParent(header),
            start: 'top 85%',
            once: true,
          },
        });
      ...
      console.error('[ToolsHeaderEditorialHeader] GSAP entrance animation failed to initialize', error);
      ctx?.revert();
      gsap.set(
        header.querySelectorAll(
          '.pf-tools-header-editorial-kicker, .pf-tools-header-editorial-title-line, .pf-tools-header-editorial-sub'
        ),
        { clearProps: 'all' }
      );
    }
  ...
  return (
    <header
      ref={headerRef}
      className={`pf-tools-header-editorial-header relative w-full ${centered ? 'text-center' : 'text-left'}`}
      style={{ marginBottom: `${TOOLS_HEADER_MARGIN_BOTTOM_REM[presentation.headerMarginBottom ?? 'md']}rem` }}
    >
      <div className={`pf-tools-header-editorial-stage flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between ${centered ? 'sm:flex-col sm:items-center' : ''}`}>
        <div className="min-w-0 max-w-3xl">
          <p
            className="pf-tools-header-editorial-kicker mb-0 text-[0.68rem] font-semibold uppercase tracking-[0.24em]"
            style={{ color: kickerInk }}
          >
            Tools
          </p>
          <h2
            className={`mb-0 mt-3 ${toolsHeaderDesignFontClass(presentation.titleFont, 'title')} ${TITLE_SIZE_CLASS[presentation.headerTitleSize ?? 'md']} ${TITLE_WEIGHT_CLASS[presentation.headerTitleWeight ?? 'regular']} tracking-[-0.03em] lg:leading-[0.98]`}
            style={toolsTitleColorStyle(presentation.titleColor)}
          >
            <span className="pf-tools-header-editorial-title-mask block overflow-hidden">
              <span className="pf-tools-header-editorial-title-line block">{titleText}</span>
            </span>
          </h2>
          {subtitleText ? (
            <p
              className={`pf-tools-header-editorial-sub mb-0 mt-6 max-w-xl ${toolsHeaderDesignFontClass(presentation.subtitleFont, 'subtitle')} text-sm leading-relaxed sm:text-base`}
              style={toolsSubtitleColorStyle(presentation.subtitleColor)}
            >
              {subtitleText}
            </p>
          ) : null}
