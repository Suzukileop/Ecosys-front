...
const DEFAULT_WORD_1 = 'Daily';
const DEFAULT_WORD_2 = 'Tools';
const MARQUEE_REPEATS = 4;
const MARQUEE_SPEED_PX = 44; // Experience's "medium" default
const MARQUEE_GAP = '1rem'; // "md" default

const WORD_SIZE: Record<PortfolioToolsHeaderTitleSize, string> = {
...
function ToolsHeaderMarqueeTrack({
  words,
  ink,
  fontSize,
  hidden = false,
 ...
 */
export function ToolsHeaderMarqueeHeader({
  ...
  const ink = toolsHeaderPaletteTokenColor(presentation.headerMarqueeWordColor ?? 'principal');
  ...
  useLayoutEffect(() => {
    ...
    const scroller = toolsHeaderScrollParent(root);
    ...
    >
      ...
      >
        <div ref={rowRef} className="flex w-max whitespace-nowrap will-change-transform">
          <ToolsHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} />
          <ToolsHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} hidden />
        </div>
      </div>
