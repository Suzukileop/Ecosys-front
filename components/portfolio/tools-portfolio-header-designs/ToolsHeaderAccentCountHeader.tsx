...
const SIZE_CLASS: Record<PortfolioToolsHeaderTitleSize, string> = {
...
const LEAD_WEIGHT_CLASS: Record<PortfolioToolsHeaderTitleWeight, string> = {
  light: 'font-normal',
  regular: 'font-medium',
 ...
 * recedes on scroll.
 */
export function ToolsHeaderAccentCountHeader({
  ...
  const badgeColor = toolsHeaderPaletteTokenColor(presentation.headerAccentCountBadgeColor ?? 'principal');
  const leadColor = toolsHeaderPaletteTokenColor(presentation.headerAccentCountLeadColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const badge = section.querySelector<HTMLElement>('[data-tools-header-badge]');
      const leadEl = section.querySelector<HTMLElement>('[data-tools-header-lead]');
  ...
  return (
    <div
      ...
      data-tools-header="accent-count"
    >
      <p
