...
const DEFAULT_SUBTITLE_TEXT = 'Reach out through any of the channels below.';
 ...
 * visible contact methods count itself, ticking up from zero) on the left,
 ...
 * never partially.
 */
export function ContactHeaderIndexHeader({
  ...
  const labelTone = contactHeaderPaletteTokenColor(presentation.headerIndexLabelColor ?? 'texteFort');
  const numberTone = contactHeaderPaletteTokenColor(presentation.headerIndexNumberColor ?? 'principal');
  const titleTone = contactHeaderPaletteTokenColor(presentation.headerIndexTitleColor ?? 'texteFort');
  const subtitleTone = contactHeaderPaletteTokenColor(presentation.headerIndexSubtitleColor ?? 'texteFort');
  ...
  return (
    <header
      ref={headerRef}
      className="pf-contact-header-index-header relative w-full text-left"
