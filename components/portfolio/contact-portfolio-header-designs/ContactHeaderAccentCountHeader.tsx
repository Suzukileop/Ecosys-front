...
const SIZE_CLASS: Record<PortfolioContactHeaderTitleSize, string> = {
...
const LEAD_WEIGHT_CLASS: Record<PortfolioContactHeaderTitleWeight, string> = {
  light: 'font-normal',
  regular: 'font-medium',
...
/**
 * Accent count — a badge (visible contact methods count) inline with a lead
 * sentence, one flowing line. Badge and lead each have their own color; the
 * line shares one size, and "weight" governs the lead's boldness (the badge
 * stays a fixed bold pill). Same entrance/scroll motion as the other headers
 * in this set — badge scales in first, then the lead fades up; the whole
 * line recedes on scroll.
 */
export function ContactHeaderAccentCountHeader({
  ...
  const badgeColor = contactHeaderPaletteTokenColor(presentation.headerAccentCountBadgeColor ?? 'principal');
  const leadColor = contactHeaderPaletteTokenColor(presentation.headerAccentCountLeadColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const badge = section.querySelector<HTMLElement>('[data-contact-header-badge]');
      const leadEl = section.querySelector<HTMLElement>('[data-contact-header-lead]');
  ...
  return (
    <div
      ...
      data-contact-header="accent-count"
    >
      <p
