...
const DEFAULT_COUNT_TEXT = '{count} ways to reach out — details below';
...
function ContactHeaderBillboardBigWord({
  text,
  tone,
  wordStyle,
}: {
  text: string;
  tone: string;
  wordStyle: PortfolioContactHeaderBillboardWordStyle;
}) {
 ...
 */
export function ContactHeaderBillboardHeader({
  ...
  const wordTone = contactHeaderPaletteTokenColor(presentation.headerBillboardWordColor ?? 'principal');
  const titleTone = contactHeaderPaletteTokenColor(presentation.headerBillboardTitleColor ?? 'principal');
  const metaTone = contactHeaderPaletteTokenColor(presentation.headerBillboardMetaColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const bigTitle = section.querySelector('[data-contact-header-gsap-big-title]') as HTMLElement | null;
      const secondaryTitle = section.querySelector('[data-contact-header-gsap-secondary]') as HTMLElement | null;
      const metaEl = section.querySelector('[data-contact-header-gsap-meta]') as HTMLElement | null;
  ...
  return (
    <div
      ...
      data-contact-header="billboard"
    >
      <div style={getInitialBigTitleStyle()} data-contact-header-gsap-big-title>
        <ContactHeaderBillboardBigWord text={bigWord} tone={wordTone} wordStyle={presentation.headerBillboardWordStyle ?? 'outline'} />
      </div>
      ...
      <div className="mt-8 flex flex-col gap-5 sm:mt-10 sm:flex-row sm:items-end sm:justify-between sm:gap-10">
        <h2
          ...
          data-contact-header-gsap-secondary
        >
          {renderSecondaryTitle}
        </h2>

        <div className="flex max-w-xs flex-col items-start gap-3 sm:items-end" style={getInitialMetaStyle()} data-contact-header-gsap-meta>
          {countText ? (
