...
function teamHeaderScrollParent(el: HTMLElement | null): HTMLElement | undefined {
 ...
 */
export function TeamHeaderEditorialHeader({
  title,
  subtitle,
  ...
  useLayoutEffect(() => {
    ...
    try {
      ctx = gsap.context(() => {
        ...
        const kicker = pick('.pf-team-header-editorial-kicker');
        const lines = pick('.pf-team-header-editorial-title-line');
        const subs = pick('.pf-team-header-editorial-sub');
        const stage = header.querySelector<HTMLElement>('.pf-team-header-editorial-stage');
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            trigger: header,
            scroller: teamHeaderScrollParent(header),
            start: 'top 85%',
            once: true,
          },
        });
      ...
      console.error('[TeamHeaderEditorialHeader] GSAP entrance animation failed to initialize', error);
      ctx?.revert();
      gsap.set(
        header.querySelectorAll(
          '.pf-team-header-editorial-kicker, .pf-team-header-editorial-title-line, .pf-team-header-editorial-sub'
        ),
        { clearProps: 'all' }
      );
    }
  ...
  return (
    <header
      ref={headerRef}
      className={`pf-team-header-editorial-header relative w-full ${centered ? 'text-center' : 'text-left'}`}
