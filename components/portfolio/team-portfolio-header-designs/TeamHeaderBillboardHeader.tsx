...
const DEFAULT_BIG_WORD = 'TEAM';
const DEFAULT_COUNT_TEXT = '{count} people — meet the team below';
...
function TeamHeaderBillboardBigWord({
  text,
  tone,
  wordStyle,
}: {
  text: string;
  tone: string;
  wordStyle: PortfolioTeamHeaderBillboardWordStyle;
}) {
 ...
 */
export function TeamHeaderBillboardHeader({
  ...
  const wordTone = teamHeaderPaletteTokenColor(presentation.headerBillboardWordColor ?? 'principal');
  const titleTone = teamHeaderPaletteTokenColor(presentation.headerBillboardTitleColor ?? 'principal');
  const metaTone = teamHeaderPaletteTokenColor(presentation.headerBillboardMetaColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const bigTitle = section.querySelector('[data-team-header-gsap-big-title]') as HTMLElement | null;
      const secondaryTitle = section.querySelector('[data-team-header-gsap-secondary]') as HTMLElement | null;
      const metaEl = section.querySelector('[data-team-header-gsap-meta]') as HTMLElement | null;
  ...
  return (
    <div
      ...
      data-team-header="billboard"
    >
      <div style={getInitialBigTitleStyle()} data-team-header-gsap-big-title>
        <TeamHeaderBillboardBigWord text={bigWord} tone={wordTone} wordStyle={presentation.headerBillboardWordStyle ?? 'outline'} />
      </div>
      ...
      <div className="mt-8 flex flex-col gap-5 sm:mt-10 sm:flex-row sm:items-end sm:justify-between sm:gap-10">
        <h2
          ...
          data-team-header-gsap-secondary
        >
          {renderSecondaryTitle}
        </h2>

        <div className="flex max-w-xs flex-col items-start gap-3 sm:items-end" style={getInitialMetaStyle()} data-team-header-gsap-meta>
          {countText ? (
