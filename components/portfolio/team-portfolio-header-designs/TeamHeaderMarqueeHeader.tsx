...
const WORD_SIZE: Record<PortfolioTeamHeaderTitleSize, string> = {
...
function TeamHeaderMarqueeTrack({
  words,
  ink,
  fontSize,
  hidden = false,
 ...
 */
export function TeamHeaderMarqueeHeader({
  ...
  useLayoutEffect(() => {
    ...
    const scroller = teamHeaderScrollParent(root);
    ...
    >
      ...
      >
        <div ref={rowRef} className="flex w-max whitespace-nowrap will-change-transform">
          <TeamHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} />
          <TeamHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} hidden />
        </div>
      </div>
