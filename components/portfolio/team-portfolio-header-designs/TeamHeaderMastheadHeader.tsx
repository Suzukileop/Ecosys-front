...
const DEFAULT_LINE_1 = 'Real people.';
const DEFAULT_LINE_2 = 'Chosen with care.';
const DEFAULT_LINE_3 = 'Built to deliver.';

const HEADLINE_SIZE_CLASS: Record<PortfolioTeamHeaderTitleSize, string> = {
...
const HEADLINE_WEIGHT: Record<PortfolioTeamHeaderTitleWeight, { light: number; heavy: number }> = {
 ...
 * line.
 */
export function TeamHeaderMastheadHeader({
  ...
  // word) creates fresh `[data-team-header-word]` spans that the effects
  ...
  useEffect(() => {
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || !animationEnabled) {
      section.querySelectorAll<HTMLElement>('[data-team-header-word]').forEach((word) => {
    ...
    const triggerAnimation = () => {
      ...
      const lineEls = section.querySelectorAll<HTMLElement>('[data-team-header-line]');
      const delay = 100;

      lineEls.forEach((line, index) => {
        const words = line.querySelectorAll<HTMLElement>('[data-team-header-word]');
      ...
      {styledLines.length > 0 ? (
        <div className={`flex flex-col gap-1 sm:gap-2 ${centered ? 'items-center' : 'items-start'}`}>
          {styledLines.map((words, lineIndex) => (
            <div key={lineIndex} className="overflow-hidden" data-team-header-line>
              <h2
