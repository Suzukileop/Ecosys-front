...
const DEFAULT_BIG_WORD = 'FAQ';
const DEFAULT_COUNT_TEXT = '{count} questions answered below';
...
function FaqHeaderBillboardBigWord({
  text,
  tone,
  wordStyle,
}: {
  text: string;
  tone: string;
  wordStyle: PortfolioFaqHeaderBillboardWordStyle;
}) {
 ...
 */
export function FaqHeaderBillboardHeader({
  ...
  const wordTone = faqHeaderPaletteTokenColor(presentation.headerBillboardWordColor ?? 'principal');
  const titleTone = faqHeaderPaletteTokenColor(presentation.headerBillboardTitleColor ?? 'principal');
  const metaTone = faqHeaderPaletteTokenColor(presentation.headerBillboardMetaColor ?? 'secondaire');
  ...
  useEffect(() => {
    ...
    const triggerAnimation = () => {
      ...
      const bigTitle = section.querySelector('[data-faq-header-gsap-big-title]') as HTMLElement | null;
      const secondaryTitle = section.querySelector('[data-faq-header-gsap-secondary]') as HTMLElement | null;
      const metaEl = section.querySelector('[data-faq-header-gsap-meta]') as HTMLElement | null;
  ...
  return (
    <div
      ...
      data-faq-header="billboard"
    >
      <div style={getInitialBigTitleStyle()} data-faq-header-gsap-big-title>
        <FaqHeaderBillboardBigWord text={bigWord} tone={wordTone} wordStyle={presentation.headerBillboardWordStyle ?? 'outline'} />
      </div>
      ...
      <div className="mt-8 flex flex-col gap-5 sm:mt-10 sm:flex-row sm:items-end sm:justify-between sm:gap-10">
        <h2
          ...
          data-faq-header-gsap-secondary
        >
          {renderSecondaryTitle}
        </h2>

        <div className="flex max-w-xs flex-col items-start gap-3 sm:items-end" style={getInitialMetaStyle()} data-faq-header-gsap-meta>
          {countText ? (
