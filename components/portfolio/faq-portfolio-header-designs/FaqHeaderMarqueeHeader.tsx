...
const DEFAULT_WORD_1 = 'Quick';
const DEFAULT_WORD_2 = 'Answers';
const MARQUEE_REPEATS = 4;
const MARQUEE_SPEED_PX = 44; // Experience's "medium" default
const MARQUEE_GAP = '1rem'; // "md" default

const WORD_SIZE: Record<PortfolioFaqHeaderTitleSize, string> = {
...
function FaqHeaderMarqueeTrack({
  words,
  ink,
  fontSize,
  hidden = false,
 ...
 */
export function FaqHeaderMarqueeHeader({
  ...
  const ink = faqHeaderPaletteTokenColor(presentation.headerMarqueeWordColor ?? 'principal');
  ...
  useLayoutEffect(() => {
    ...
    const scroller = faqHeaderScrollParent(root);
    ...
    >
      ...
      >
        <div ref={rowRef} className="flex w-max whitespace-nowrap will-change-transform">
          <FaqHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} />
          <FaqHeaderMarqueeTrack words={displayWords} ink={ink} fontSize={fontSize} hidden />
        </div>
      </div>
