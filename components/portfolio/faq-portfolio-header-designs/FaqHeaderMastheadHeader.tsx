...
const DEFAULT_LINE_1 = 'Got questions.';
const DEFAULT_LINE_2 = 'We have answers.';
const DEFAULT_LINE_3 = 'Read before reaching out.';

const HEADLINE_SIZE_CLASS: Record<PortfolioFaqHeaderTitleSize, string> = {
...
const HEADLINE_WEIGHT: Record<PortfolioFaqHeaderTitleWeight, { light: number; heavy: number }> = {
 ...
 * line.
 */
export function FaqHeaderMastheadHeader({
  ...
  const ink = faqHeaderPaletteTokenColor(presentation.headerMastheadHeadlineColor ?? 'principal');
  ...
  // word) creates fresh `[data-faq-header-word]` spans that the effects
  ...
  useEffect(() => {
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || !animationEnabled) {
      section.querySelectorAll<HTMLElement>('[data-faq-header-word]').forEach((word) => {
    ...
    const triggerAnimation = () => {
      ...
      const lineEls = section.querySelectorAll<HTMLElement>('[data-faq-header-line]');
      const delay = 100;

      lineEls.forEach((line, index) => {
        const words = line.querySelectorAll<HTMLElement>('[data-faq-header-word]');
      ...
      {styledLines.length > 0 ? (
        <div className={`flex flex-col gap-1 sm:gap-2 ${centered ? 'items-center' : 'items-start'}`}>
          {styledLines.map((words, lineIndex) => (
            <div key={lineIndex} className="overflow-hidden" data-faq-header-line>
              <h2
